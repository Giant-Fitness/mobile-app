...
const SingleDataPointState = ({ measurement, onPress, themeColors }: { measurement: UserSleepMeasurement; onPress: () => void; themeColors: any }) => {
...
export const SleepOverviewChartCard: React.FC<SleepOverviewChartCardProps> = ({ values, onPress, onLogSleep, isLoading = false, style = {} }) => {
    ...
    const { processedData, dateRange, averageSleep, points } = useMemo(() => {
        if (!values?.length) {
            return { processedData: '', dateRange: '', averageSleep: 0, points: [] };
        }
        ...
        if (recentData.length < 2) {
            return {
                ...
                averageSleep: recentData[0]?.DurationInMinutes || 0,
                points: [],
            };
        }
        ...
        const chartPoints = recentData.map((d, i) => {
            ...
            const y = 40 - PADDING_VERTICAL - ((d.DurationInMinutes - adjustedMinSleep) / sleepRange) * (40 - 2 * PADDING_VERTICAL);
            return { x, y, sleep: d.DurationInMinutes };
        });
