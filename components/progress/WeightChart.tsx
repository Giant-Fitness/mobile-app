    ...
    import { View, StyleSheet, Dimensions, TouchableWithoutFeedback, TouchableOpacity, MeasureInWindowOnSuccessCallback } from 'react-native';
    ...
    export const WeightChart: React.FC<WeightChartProps> = ({
        ...
        const isWeightMeasurement = (data: any): data is UserWeightMeasurement => {
            return data && typeof data.weight === 'number';
        };
        ...
        //         const value = isWeightMeasurement(point.originalData) ? point.originalData.Weight : point.originalData.DurationInMinutes;
        ...
        return data.map((point) => {
            const value = isWeightMeasurement(point.originalData)
                ? point.originalData.weight
                : point.originalData.durationInMinutes;
    
            return {
                x: CHART_PADDING.left + ((point.timestamp.getTime() - timeStart) / timeRange) * plotWidth,
                y: CHART_PADDING.top + ((yAxisRange.max - value) / (yAxisRange.max - yAxisRange.min)) * plotHeight,
                value, // Add the value dynamically for generalization
                timestamp: point.timestamp,
                originalData: point.originalData,
            };
        });
        ...
        const renderTooltip = () => {
            ...
            const value = isWeightMeasurement(selectedPoint.originalData)
            ? `${selectedPoint.weight?.toFixed(1)} kg`
            : `${selectedPoint.durationInMinutes} minutes`;
